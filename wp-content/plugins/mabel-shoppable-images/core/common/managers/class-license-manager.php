...
namespace MABEL_SI\Core\Common\Managers
{
	...
	class License_Manager
	{
		...
		public function init() {
			...
			add_action( 'admin_post_' .$this->slug. '-activate-license', array($this, 'activate_license'));
			add_action( 'admin_post_' .$this->slug. '-deactivate-license', array($this, 'deactivate_license'));
		...
		public static function get_license_info() {
			$raw = get_option(Config_Manager::$slug.'_license');
		...
		public function deactivate_license()
		{
			...
			wp_redirect(admin_url('options-general.php?page='.$this->slug . '&notice='.base64_encode(serialize($notice))));
		}

		public function activate_license()
		{
			$key = $_REQUEST[$this->slug.'-license'];
			...
			if($result === null){
				$notice = array(
					'class' => 'error',
					'message' => "Something went wrong. Can't connect to the license server."
				);
				//wp_redirect(admin_url("options-general.php?page=" . Config_Manager::$slug.'&notice=success&m='.__('Can not contact server',Config_Manager::$slug)));
				//exit;
			} else{
		...
		public function set_version_info_cache( $value ) {
			...
			set_transient($this->slug .'_version_info', json_encode($value), HOUR_IN_SECONDS * 6 ); // Store for 8 hours.
		...
		private function api_request( $url ) {
			...
			$data = array(
				'timeout' 	=> apply_filters('shoppable_images_license_timeout', 5),
				'body'		=> $api_params
			);
