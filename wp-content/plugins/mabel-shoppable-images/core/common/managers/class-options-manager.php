...
namespace MABEL_SI\Core\Common\Managers
{
	...
	use MABEL_SI\Core\Models\Hidden_Option;
	use MABEL_SI\Core\Models\Option;
	use MABEL_SI\Core\Models\Option_Section;
	use Exception;

	/**
	 * Register all options to show in admin screens.
	 * Class Options_Manager
	 * @package MABEL_SI\Core
	 */
	class Options_Manager
	{
		/**
		...
		public function add_option($section_id, Option $option)
		{
			...
			if($option instanceof Container_Option)
				foreach($option->options as $op){
					$op->name = $this->set_option_name($op);
				}
			else $option->name = $this->set_option_name($option);
