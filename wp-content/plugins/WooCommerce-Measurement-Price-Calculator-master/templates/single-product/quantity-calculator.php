...
<table id="price_calculator" class="wc-measurement-price-calculator-price-table <?php echo sanitize_html_class( $product->get_type() . '_price_calculator' ) . ' ' . sanitize_html_class( $calculator_mode ); ?>">
	...
	<?php foreach ( $measurements as $measurement ) : ?>
		...
		<?php if ( $measurement->is_editable() ) : ?>
			...
			<tr class="price-table-row <?php echo sanitize_html_class( $measurement->get_name() ); ?>-input">
				...
				<td>
					<span id="calculateQty">Calcular</span>
				</td>
			</tr>
		<?php endif; ?>
	...
	<tr>
		<td id="calculationResult" style="display:none;"  colspan="3">
			<p>Necesita <span id="boxesAmount"></span> caja<span id="manyBoxesAmount">s</span> para cubrir la superficie deseada.
			<br>
			<strong><?php esc_html_e( 'Total Price', 'woocommerce-measurement-price-calculator' ); ?>: <span class="total_price" style="margin-bottom:25px;margin-top:5px;"><?php echo $total_price; ?></span></strong></p>
		</div>
	</tr>
